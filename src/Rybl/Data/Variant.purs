...
instance (RowToList r rl, EncodeJsonVariantRL r rl) => EncodeJson (Variant r) where
  encodeJson = encodeJsonVariantRL @r @rl

class EncodeJsonVariantRL (r :: Row Type) (rl :: RL.RowList Type) | rl -> r where
  encodeJsonVariantRL :: Variant r -> Json

instance EncodeJsonVariantRL () RL.Nil where
  encodeJsonVariantRL = case_

instance
  ( EncodeJsonVariantRL r rl
  , IsSymbol x
  , EncodeJson a
  , Cons x a r r'
  ) =>
  EncodeJsonVariantRL r' (RL.Cons x a rl) where
  encodeJsonVariantRL =
    on (Proxy @x)
      (\a -> encodeJson { label: reflectSymbol (Proxy @x), value: a })
      (encodeJsonVariantRL @r @rl)

instance (RowToList r rl, DecodeJsonVariantRL r rl) => DecodeJson (Variant r) where
  decodeJson = decodeJsonVariantRL @r @rl

class DecodeJsonVariantRL (r :: Row Type) (rl :: RL.RowList Type) | rl -> r where
  decodeJsonVariantRL :: Json -> JsonDecodeError \/ Variant r

instance DecodeJsonVariantRL () RL.Nil where
  decodeJsonVariantRL _ = throwError $ TypeMismatch $ "didn't match any form of variant"

instance
  ( DecodeJsonVariantRL r rl
  , IsSymbol x
  , DecodeJson a
  , Cons x a r r'
  ) =>
  DecodeJsonVariantRL r' (RL.Cons x a rl) where
  decodeJsonVariantRL json = do
    { label, value } <- json # decodeJson @{ label :: String, value :: Json }
    if label == reflectSymbol (Proxy @x) then do
      inj (Proxy @x) <$> decodeJson @a value
    else
      expandCons @x <$> decodeJsonVariantRL @r @rl json

reflectVariantKey :: forall r. Variant r -> String
reflectVariantKey v = v'.type
  where
  VariantRep v' = unsafeCoerce v :: VariantRep VariantCase

expandCons :: forall @x a r r'. Cons x a r r' => Variant r -> Variant r'
